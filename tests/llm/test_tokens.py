...
def test_truncate_normal() -> None:
    ...
    assert truncate_str(text, max_len, model="gpt-3.5-turbo-0613") == "This is a"


def test_truncate_no_truncation() -> None:
    text = "Short text"
    max_len = 20
    assert truncate_str(text, max_len, model="gpt-3.5-turbo-0613") == text


def test_truncate_empty_string() -> None:
    text = ""
    max_len = 5
    assert truncate_str(text, max_len, model="gpt-3.5-turbo-0613") == ""
...
def test_num_tokens_in_string() -> None:
    text = "This is a test sentence for the function."
    assert num_tokens_in_string(text, model="gpt-3.5-turbo-0613") == 9
...
def test_num_tokens_single_message() -> None:
    ...
    assert num_tokens_from_messages(messages, model="gpt-3.5-turbo-0613") == 9


def test_num_tokens_multiple_messages() -> None:
    messages = [{"role": "system", "content": "System message."}, {"role": "user", "content": "User message."}]
    assert num_tokens_from_messages(messages, model="gpt-3.5-turbo-0613") == 17
...
def test_num_tokens_with_names() -> None:
    ...
    assert num_tokens_from_messages(messages, model="gpt-3.5-turbo-0613") == 19
